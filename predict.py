...
model = load_model(pathlib.Path('DMR_fraude/depot/TMP/model/model_ame.h5'))
model.summary()
...
for i in file:
    ...
    if classes == 'Facture_fausse':
        ...
        shutil.move(str(pathlib.Path('DMR_fraude/depot/'))+"/"+name_b+".pdf", file_destination)
        shutil.move(str(pathlib.Path('DMR_fraude/depot/'))+"/"+name_b+".xml", file_destination)
    else: 
        #print(i)
        #name = i.split("__")[0]
        #print(i)
...
# capter les contours sur une facture quelconque
...
# fonction pour faire sortir les dates
...
for i in file:
    ...
    try:
        if dateferiee(liste)[0] in jour_ferie:
            ...
            name_b =str(name).split("__")[0]
            try :
                shutil.move(str(pathlib.Path('DMR_fraude/'))+"/"+name_b+".pdf", file_destination)
                shutil.move(str(pathlib.Path('DMR_fraude/'))+"/"+name_b+".xml", file_destination)
            except:
...
# fonction pour detecter les  ro sur des factures non soumises aux Ro
def appartient(elt, lst):
    for e in lst:
        if e in elt:
            for i in lst:
...
# detection  ro sur des factures non soumises aux Ro  
for i in file:
    text_coordinates = detect_text_blocks(i)
    recognition_results = reader.recognize(i,
                                 horizontal_list=text_coordinates,
    ...
    for txt in recognition_results:
        tr.append((txt[1]))
    if appartient(["Part Ro","Ro","Régime Obligatoire","regime obligatoire","PART RO","RO","REGIME OBLIGATOIRE","ro"], tr) == "soupçon de fraude":
        name= os.path.basename(i)
        name_b =str(name).split("__")[0]
        try :
            shutil.move(str(pathlib.Path('DMR_fraude/'))+"/"+name_b+".pdf", file_destination)
